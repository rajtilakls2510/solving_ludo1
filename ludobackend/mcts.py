...
class MCTNode:
    def __init__(self, state, players, model, parent):
        ...
        if not self.state["game_over"]:
            self.moves = self.model.all_possible_moves(self.state)
            for d in self.moves:
                if len(d["moves"]) > 0:
                    for move in d["moves"]:
                        self.available_moves.append({"roll": d["roll"], "move": move})
                else:
                    self.available_moves.append({"roll": d["roll"], "move": [[]]})
...
def mcts_job(num, root, player, evaluator_conn, c_puct, n_vl, prior_temp):
    ...
    try:
        ...
        if not node.expansion_event.is_set():
            # In the unfortunate case that a thread has already got passes event.wait() while another thread is expanding the same node, discard the thread
        ...
        if not node.state["game_over"]:
            ...
            for move in node.available_moves:
                node.state["dice_roll"] = move["roll"]
                next_states.append(node.model.generate_next_state(node.state, move["move"]))
            exp_mid = time.perf_counter()
            print(f"{num} Next states Generated {len(next_states)} in {exp_mid - exp_start}")
            node.children = [MCTNode(state, node.players, node.model, node) for state in next_states]
            exp_end = time.perf_counter()
            print(f"{num} Nodes Generated {len(node.children)} in {exp_end - exp_mid} ")
        node.expanded = True
        ...
        if not node.state["game_over"]:
            states_serialized = base64.b64encode(
                tf.io.serialize_tensor(tf.stack([node.model.state_to_repr(state) for state in next_states])).numpy()).decode(
                'ascii')
        ...
        print(f"{num} Num moves: {len(move_indices)} Ending: {chk5 - chk4}")
